...
def test_download_documents_handle_stubbed_response(mock_post, mock_fatal_error):
    # given:
    camunda = Camunda()
    
    mock_response = Mock()
    mock_response.status_code = 501
    mock_response.raise_for_status = Mock(side_effect=HTTPError("Response was not 200 or 501 (Should NOT see this)"))
    mock_post.return_value = mock_response
    
    # and:
    mock_fatal_error.side_effect = AssertionError("fatal_error was called on mock (Should see this)")
